...
def main():
    ...
    _,  act_len3, nb_filter3 = model.nodes['conv3'].output_shape
    ...
    counts3 = np.zeros((nb_filter3, filter_len3, channel_num))+1e-5
    ...
    while i+BATCH_SIZE < N:
        ...
        counts3 = update_counts(counts3, x, a3)
    ...
    pwm3 = counts3/counts3.sum(axis=2).reshape(nb_filter3, filter_len3, 1)
    ...
    for i in range(0, nb_filter3):
        outfile.write('MOTIF FILTER_LEN%s_%s\n\n' % (filter_len3, i))
        outfile.write('letter-probability matrix: alength= 4\n')
        
        for j in range(0, filter_len3):
            outfile.write('%f\t%f\t%f\t%f\n' % tuple(pwm3[i, j, :].tolist()))
